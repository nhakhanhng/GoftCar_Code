...
const osThreadAttr_t CANRcvTask_attributes = { .name = "CANRcvHandler",
		.stack_size = 500 * 4, .priority = (osPriority_t) osPriorityHigh5, };
...
//	SyncPrintf("Rcv Frame Type %d CurrentFrameType %d \r\n", RcvFrameType, CurrentFrameType);
	...
	uint8_t Calc_CRC;
//	switch (MsgType) {
//	case CANFRAME_MSGTYPE_ACK_COMMAND_FRAME:
//		uint32_t counter = htim1.Instance->CNT;
//		SyncPrintf("Time: %ld\r\n", counter);
...
//				txHeader.MessageType = CANFRAME_MSGTYPE_ACK_COMMAND_FRAME;
//				txHeader.TargetNode = CANFRAME_ALL_NODE;
//				txHeader.DataLen = 4;
//				uint8_t *Msg = "ACK\n";
//				CANFrame_Send(CANHandler, &txHeader, Msg, 20);
			} else {
